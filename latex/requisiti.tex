...
\begin{itemize}
  ...
  \item[-] Il farmacista vede le prenotazioni effettuate nella propria farmacia e la lista dei farmaci, con specificato quelli in scadenza o in esaurimento
...
\hline
  R2F & Specifica da parte dell'utente del farmaco da cercare e della città in cui cercarlo & Funzionale \\
\hline
...
\hline
  R10F & Conferma della prenotazione andata a buon fine & Funzionale\\
\hline
  R11F & Visualizzazione dei farmaci e notifica dei farmaci in scadenza e in esaurimento & Funzionale\\
\hline
...
\hline
  Blocco & Blocco di un utente a seguito di prenotazioni non concluse & Sospensione \\
\hline
\end{tabular}
...
\hline
  \textbf{Scenario Principale} & 1. Login \linebreak 2. Il farmacista può
  eseguire la verifica e la sospensione di un account, controllare le prenotazioni e i
  farmaci disponibili, in scadenza o in esaurimento \\
\hline
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}
  ...
  \hline
    ...
    visualizzazione farmaci \linebreak 2. Il sistema recupera l'elenco dei
    farmaci in esaurimento o in scadenza \linebreak 3. Il sistema mostra a video
    l'elenco richiesto\\
  \hline
    \textbf{Scenari Alternativi} &\\
  \hline
...
\hline
  \textbf{Descrizione} & Si controllano gli utenti potenzialmente sospendibili\\
\hline
  \textbf{Attori} & Farmacista\\
\hline
...
\hline
  \textbf{Scenario Principale} & 1. Il farmacista va nella schermata di
  visualizzazione utenti \linebreak 2. Il sistema recupera l'elenco degli
  utenti a rischio o sospesi \linebreak 3. Il sistema mostra a video l'elenco
  degli utenti\\
\hline
  \textbf{Scenari Alternativi} &\\
\hline
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}
  ...
  \hline
    \textbf{Postcondizioni} & Il cliente non può più effettuare prenotazioni finchè non viene sbloccato\\
  \hline
    \textbf{Scenario principale} & 1. ControlloUtenti \linebreak 2. Il
...
\hline
  \textbf{Scenario principale} & 1. Il farmacista va nella schermata di
  visualizzazione prenotazioni \linebreak 2. Il sistema recupera l'elenco delle
  prenotazioni giornaliere \linebreak 3. Il sistema mostra a video l'elenco delle
  prenotazioni\\
\hline
  \textbf{Scenari Alternativi} & 1. Il farmacista va nella schermata di
  visualizzazione delle prenotazioni \linebreak 2. Il farmacista specifica
  l'intervallo di tempo nel quale recuperare le prenotazioni \linebreak 3. Il
  sistema recupera l'elenco delle prenotazioni richiesto \linebreak 4. Il
  sistema mostra a video l'elenco delle prenotazioni \\
\hline
...
\hline
  \textbf{Scenario principale} & 1. ControlloPrenotazioni \linebreak 2. Il farmacista conferma l'avvenuta prenotazione\\
\hline
  \textbf{Scenari Alternativi} & \\
\hline
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}
  ...
  \hline
    ...
    documento specificato in fase di registrazione \linebreak 2. Il cliente viene
    identificato dal farmacista \linebreak 3. Il farmacista chiede al sistema di
    recuperare l'utente \linebreak 4. Il farmacista attiva l'account dell'utente\\
  \hline
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}
  ...
  \hline
    \textbf{Scenario principale} & 
      1. Il cliente si reca nella pagina di ricerca \linebreak
      2. Il cliente inserisce il nome del farmaco per cui eseguire la ricerca \linebreak
      3. Il sistema ottiene la lista delle farmacie aventi il farmaco specificato entro un range dalla località specificata. \linebreak
      4. Il sistema ordina la lista in base alla distanza geografica dalla zona dell'utente, in ordine crescente \linebreak
      5. La lista viene mostrata all'utente\\
  \hline
    \textbf{Scenari Alternativi} &
      Scenario alternativo A: \linebreak
        1. Login \linebreak 
        2. Il cliente si reca nella pagina di ricerca \linebreak
        3. Il cliente inserisce il nome del farmaco per cui eseguire la ricerca \linebreak
        4. Il sistema ottiene la lista delle farmacie aventi il farmaco specificato entro un range dalla località specificata. \linebreak
        5. Il sistema ordina la lista in base alla distanza geografica dalla zona dell'utente, in ordine crescente \linebreak
        6. La lista viene mostrata all'utente \linebreak
        7. L'utente può selezionare il farmaco per cominciare una prenotazione di quel farmaco nella farmacia scelta \linebreak
      Scenario alternativo B: \linebreak
        1. NuovaPrenotazione scenario alternativo A \linebreak
        2. Il cliente inserisce il nome del farmaco per cui eseguire la ricerca \linebreak
        3. Il sistema ottiene la lista dei farmaci disponibili nella farmacia scelta il cui nome inizia per il testo inserito dall'utente \linebreak
        4. La lista viene mostrata all'utente \linebreak
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}\hline
  ...
  \hline
    \textbf{Postcondizioni} &\\
  \hline
    \textbf{Scenario Principale} & 1. Il cliente esegue il login \linebreak 2. Il
...
\hline
  \textbf{Scenario principale} &
    1. RicercaFarmaci scenario alternativo A \linebreak 2. Il cliente può
    selezionare altri farmaci che vuole prenotare, la quantità, e inserisce la
    data di ritiro desiderata \linebreak 3. Il cliente invia la richiesta di
    prenotazione \linebreak 4. Il sistema pone la richiesta in attesa di
    conferma \\
\hline
  \textbf{Scenari Alternativi} & 
    Scenario A:
      1. Login \linebreak
      2. Il cliente seleziona l'opzione "nuova prenotazione" \linebreak
      3. Il cliente seleziona la farmacia e inserisce la data di ritiro desiderata \linebreak
      4. Il cliente seleziona i farmaci che vuole prenotare e le relative quantità\linebreak
      5. Il cliente invia la richiesta di prenotazione \linebreak
      6. Il sistema pone la richiesta in attesa di conferma\linebreak
    Scenario B: La farmacia non dispone dei farmaci richiesti. \linebreak
    1. Il sistema nota che la farmacia non ha disponibilità di almeno uno dei farmaci specificati \linebreak
...
\hline
  \textbf{Scenario Principale} & 1. Login \linebreak 2. Il cliente seleziona
  l'opzione di visualizzazione della lista delle prenotazioni \linebreak 3. Al cliente
...
\begin{tabular} {|P{4.5cm}|P{12.5cm}|}
  ...
  \hline
    ...
    registrazione \linebreak 2. Il cliente inserisce i propri dati: nome,
    cognome, data di nascita, email, password e il codice fiscale \linebreak 3. Il
  ...
  \hline
    \textbf{Scenari Alternativi} & 1. Il cliente accede
    alla sezione di registrazione \linebreak 2. Il cliente inserisce i propri
    dati: nome, cognome, data di nascita, email, password e codice fiscale.
    \linebreak 3. Il sistema verifica che è già presente un utente
...
\hline
  \textbf{Scenario principale} & 1. Si verifica l'evento FineGiornata\linebreak
  2. Il sistema controlla le prenotazioni non andate a buon fine \linebreak 3. Il
...
\hline
  ...
  \linebreak 2. Il sistema recupera le modifiche dal DataBase Remoto \linebreak 3. Il
  sistema aggiorna i dati relativi ai farmaci in magazzino\\
\hline
  \textbf{Scenari Alternativi} &\\
\hline
  \textbf{Requisiti non funzionali} & Velocità della ricerca dei dati\\
...
\hline
  Informazioni dei clienti & Alto. Dati generali dei clienti della farmacia,
  comprese le credenziali & Alta. Perdita di immagine dovuta alla divulgazione
  di dati sensibili\\
\hline
  Informazioni relative al personale & Alto. Dati relativi ai farmacisti,
  incluse le credenziali di accesso all'area riservata & Molto Alta. Perdita
  finanziaria dovuta a usi impropri delle credenziali con privilegi elevati.
  Perdita di immagine possibile con la divulgazione dei dati relativi ai
  clienti\\
\hline
  Dati delle prenotazioni & Alto. Necessario per tenere traccia delle
  prenotazioni & Molto Alta. Perdita finanziaria dovuta allo smarrimento di
  prenotazioni. Perdita di immagine con la divulgazione dei farmaci prenotati
  dai clienti\\
\hline
\end{tabular}
...
\hline
  Saturazione del database & Bassa & 1. Limitazione delle richieste in un dato intervallo di tempo. \linebreak 2. Limite di tempo per la verifica di un cliente & Media complessità di implementazione\\
\hline
\end{tabular}
...
\hline
  \textbf{ID} & \textbf{Requisiti} & \textbf{Tipo}\\
\hline
  R16F & Implementazione di un sistema di log per tracciare tutti i messaggi
  tra i client e i server, inclusi gli accessi, le richieste di prenotazione,
  di conferma, di sospensione e di invio e ricezione di dati & Funzionale\\
\hline
